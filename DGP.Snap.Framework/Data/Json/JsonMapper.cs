    ...
    public class JsonMapper
    {
        ...
        private static void AddObjectMetadata(Type type)
        {
            ...
            foreach (PropertyInfo p_info in type.GetProperties(BindingFlags.Instance | BindingFlags.Public))
            {
                ...
                if (p_info.IsDefined(typeof(JsonProperty), true))
                {
                    var att = (JsonProperty)p_info.GetCustomAttributes(typeof(JsonProperty), true)[0];
            ...
            foreach (FieldInfo f_info in type.GetFields(BindingFlags.Instance | BindingFlags.Public))
            {
                ...
                if (f_info.IsDefined(typeof(JsonProperty), true))
                {
                    var att = (JsonProperty)f_info.GetCustomAttributes(typeof(JsonProperty), true)[0];
                    data.Properties.Add(att.Name, p_data);
                }
                else
                {
        ...
        private static void ReadSkip(JsonReader reader) => ToWrapper(
        ...
        public static dynamic ToObject(JsonReader reader, Type type) => ReadValue(type, reader);
